...
class Rank extends MY_Controller {
    ...
    public function new_rank() {
        ...
        $this->form_validation->set_rules('st', 'short tag', 'trim|required|alpha|is_unique[m_rank.rank_short]');

        $this->form_validation->set_rules('status', 'status', 'trim|required|in_list[0,1]');

        if ($this->form_validation->run()) {
            $this->RM->rank_desc = $this->input->post('desc');
            $this->RM->rank_short = $this->input->post('st');
